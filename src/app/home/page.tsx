...
} from "@/utils/parseSearchResponse";
...
export default function HomePage() {
  const [activeTab, setActiveTab] = useState<"wardrobe" | "profile">(
    "wardrobe"
  );
  ...
  const [searchResult, setSearchResult] = useState<ParsedSearchResponse | null>(
    null
  );
  const [editingItem, setEditingItem] = useState<WardrobeItem | null>(null);
  const [editDescription, setEditDescription] = useState("");
  const [expandedSuggestion, setExpandedSuggestion] = useState(false);
  const [expandedMissingItems, setExpandedMissingItems] = useState(false);
                              ...
                              <div className="text-gray-800 prose prose-sm max-w-none">
                                {expandedSuggestion ? (
                                  ...
                                  <div
                                    ...
                                    onClick={() => setExpandedSuggestion(true)}
                                  >
                                    <ReactMarkdown>
                                      {truncateText(searchResult.suggestions)}
                                    </ReactMarkdown>
                                  </div>
                                )}
                              </div>
                          ...
                          {searchResult.recommendedItems.length > 0 && (
                            <div className="p-4 bg-blue-50 rounded-lg">
                              <h4 className="font-medium text-gray-900 mb-3">
                                Recommended Items from Your Closet
                              </h4>
                              <div className="grid gap-3">
                                {searchResult.recommendedItems.map(
                                  (rec, index) => (
                                    <div
                                      key={index}
                          ...
                          {searchResult.missingItems && (
                            <div className="p-4 bg-yellow-50 rounded-lg">
                              <div className="flex justify-between items-center mb-3">
                                ...
                                <button
                                  onClick={() =>
                                    setExpandedMissingItems(
                                      !expandedMissingItems
                                    )
                                  }
                                  className="text-xs text-gray-600 hover:text-gray-900 underline"
                                >
                              ...
                              <div className="text-left text-gray-700 leading-relaxed prose prose-sm max-w-none [&>p]:mb-2 [&>ul]:mb-2 [&>ul]:pl-4 [&>li]:mb-1">
                                {expandedMissingItems ? (
                                  ...
                                  >
                                    ...
                                    >
                                      {truncateText(searchResult.missingItems)}
                                    </ReactMarkdown>
                                  </div>
                                )}
                              </div>
                ...
                {wardrobeItems.length === 0 ? (
                  <div className="bg-white rounded-xl p-12 shadow-sm text-center">
                    ...
                    <p className="text-gray-500 mb-6">
                      Start building your digital wardrobe by adding some
                      clothes
                    </p>
                    <Button
                      onClick={() =>
